...
| 店舗 | ①悩み | ②決意 | ③達成 | ④当日 | ⑤その後 |
|---|---|---|---|---|---|
| ota-gunma | dress_counseling_01 | effort_couple_01 | eve_01 | toast_01 | album_01 |

> ota-gunma=群馬県初出店・近接NEXUS店なし（カニバリ無関係）。omiya（埼玉）と全段で非重複になるよう別プールから選定。ブライダルは st-band で本文昇格（統合版）。

### 栃木

| 店舗 | ①悩み | ②決意 | ③達成 | ④当日 | ⑤その後 |
|---|---|---|---|---|---|
| fukui-ashikaga | commit_group_01 | ring_glove_01 | eve_02 | bouquet_01 | frame_01 |

> fukui-ashikaga=栃木県初出店（足利市・福居店）。隣接の ota-gunma（群馬・太田）と両毛エリアで面化するため、太田・大宮と全段で非重複になるよう別プールから選定。ブライダルは st-band で本文昇格（統合版）。
